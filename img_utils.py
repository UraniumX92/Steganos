...
import binary_utils
from steg_secret import SIGNATURE_TEXT,PREFIX_TEXT
import utils
import PIL
import os

# TODO : ignore the 0,0 pixel
...
def hide_data(img:Image.Image, message:str, key:str, enc_tup: tuple) -> Image.Image :
    ...
    (i,j) = (ti,tj) if (ti is not None) else (0,0) # using the remembered values if key is included, or else starting from (0,0) if key is not included in image
    temptxt = e_msg
    dict3pix = {}
    if orientation==0:
        while i < width and len(temptxt)!=0:
            while j < height and len(temptxt)!=0:
                pxl = (i,j)
                if pxl != (ti,tj) and pxl != ignored_pixel: # if current pixel is not the last pixel put while embedding key in image
                    dict3pix[pxl] = img.getpixel(pxl)
                    if len(dict3pix.keys()) == 3:
                        char = temptxt[0]
                        stop = len(temptxt) == 1
